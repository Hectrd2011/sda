"""Renders the 1950 Puerto Rican Nationalist uprisings map animation.

Usage:
    python3 render_pr.py info
    python3 render_pr.py preview "1950-10-30 12:00" [--w 1920 --h 1080]
    python3 render_pr.py video --w 3840 --h 2160 --jobs 4 --out build/pr1950_4k.mp4
"""
import argparse
import math
import os
...
def pace_multiplier(day):
    t = T.date_at(day)
    if T.parse("1950-10-30 09:00") <= t < T.parse("1950-10-30 19:00"):
        return 0.35   # the morning and afternoon of the uprising, hour by hour
    if T.parse("1950-10-30 19:00") <= t < T.parse("1950-11-02 18:00"):
        return 0.8
    return 1.6        # the evening before and the aftermath
...
class Renderer:
    ...
    def number_size(self, v):
        return min(15.0, 10.0 + 5.0 * math.sqrt(v / 7_000)) * self.s
    ...
    def draw_labels(self, img, day):
        ...
        for lab in self.labels:
            ...
            alpha = min(1.0, (day - lab["army"][0][0] + 0.02) / 0.06)
            if v <= 0 or alpha <= 0:
                continue
    ...
    def _faction_visible(self, k, day):
        for key, hist in T.OWNERS.items():
            ...
            if cur == k:
                return True
        return any(z.get("occ", "NAT") == k and T.as_day(z["start"]) <= day < T.as_day(z["end"]) for z in T.ZONES)
    ...
    def draw_intro(self, img, vt):
        ...
        d = ImageDraw.Draw(ov)
        for txt, f, y, c in [("THE NATIONALIST UPRISINGS", self.font("LiberationSans-Bold.ttf", 58 * s), 0.32, 255),
                             ("Puerto Rico, 1950 - Hour by Hour with Force Sizes",
                              self.font("LiberationSans-Bold.ttf", 24 * s), 0.46, 230),
                             ("29 October  -  3 November 1950", self.font("LiberationSans-Regular.ttf", 17 * s),
                              0.54, 200)]:
    ...
    def draw_outro(self, img, vt, sched):
        ...
        A = int(255 * a)
        if t < 10.0:
            txt = "The 1950 Nationalist uprisings"
            ...
            a3 = int(255 * min(1.0, (t - 10.0) / 1.0) * min(1.0, max(0.0, (OUTRO - t) / 1.5)))
            notes = ["Notes",
                     "Force sizes are rounded estimates; times of day are approximate.",
                     "Areas held by the Nationalists are simplified to the towns and hills around them.",
                     ...
                     "Map data: Natural Earth; elevation: AWS Terrain Tiles (SRTM and others)."]
...
def build_audio(sched, out_wav):
    import wave
    cues = [(0.0, "intro"), (sched.vt_of(T.as_day("1950-10-30 09:00")), "tension"),
            (sched.vt_of(T.as_day("1950-10-31 12:00")), "lament"), (sched.vt_of(T.as_day("1950-11-02 16:00")), "dark")]
    music = audio.make_music(sched.total, cues, seed=31)
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(BUILD, "pr1950.mp4"))
    args = ap.parse_args()
    sched = load_schedule()
    if args.mode == "info":
