...
def pace_multiplier(day):
    ...
    slow = [("2022-02-24", "2022-04-10", 0.3),   # the invasion and the battle for Kyiv
            ("2022-09-05", "2022-09-16", 0.45),  # the Kharkiv counteroffensive
            ("2022-11-08", "2022-11-13", 0.6),   # Kherson
            ("2024-08-05", "2024-08-20", 0.5)]   # the Kursk incursion
    for a, b, m in slow:
        if T.D(a) <= d < T.D(b):
            return m
    return 1.0
...
# ----------------------------------------------------------------------------- renderer
class Renderer:
    def __init__(self, W, H, lite=False, view="ukraine"):
        self.W, self.H = W, H
        self.s = W / 1280.0
        self.view_name = view
        ...
        self.fac_code = {None: 0, "UA": 1, "RU": 2, "OCC": 3, "BY": 4}
        self._victim_cache = {}
        self._mask_cache = {}
    ...
    def number_size(self, v):
        ...
        return min(26.0, 15.0 + 9.0 * math.sqrt(v / 750_000)) * self.ls * self.s
    ...
    def draw_intro(self, img, vt):
        ...
        sub = ("All Fronts of the World - Every Day with Army Sizes" if self.world
               else "Every Day - with Troop Numbers")
        for txt, f, y, c in [("RUSSIAN INVASION OF UKRAINE", f1, H * 0.34, (255, 255, 255)),
                             (sub, f2, H * 0.47, (230, 230, 230)),
                             ("24 February 2022  -  September 2026", f3, H * 0.55, (200, 200, 200))]:
    ...
    def draw_outro(self, img, vt, sched):
        ...
        A = int(255 * a)
        if t < 13.0:
            txt = "THE WAR IN UKRAINE  2022 - 2026"
            ...
            notes = [
                "Notes",
                "Troop numbers are rounded public estimates; the real figures are secret and disputed.",
                "Front lines are approximate and simplified; grey zones and small salients are not shown.",
                "Front lines after mid-2026 are based on news reports and ISW assessments up to 12 September 2026.",
                "Speeches: original recordings. Music: see the video description.",
...
def load_schedule():
    """Speeches: the original recordings in build/real/speeches/<file>.wav when present,
    otherwise an AI-voice stand-in."""
    import soundfile as sf
    meta = audio.make_speeches(T.SPEECHES)
    for s in T.SPEECHES:
        wav = os.path.join(REAL, "speeches", s.get("file", "") + ".wav")
        if s.get("file") and os.path.exists(wav):
...
MUSIC = [("01_behemoth.wav", 10.7),                   # GosT - Behemoth (Perturbator remix), after the spoken intro
         ...
         ("05_end_of_the_war.wav", 146.3)]            # Hearts of Iron IV - The End of the War, last part to the finish
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(HERE, "build", "ukraine.mp4"))
    ...
    ap.add_argument("--view", choices=["ukraine"], default="ukraine")
    args = ap.parse_args()
    sched = load_schedule()
    if args.mode == "info":
